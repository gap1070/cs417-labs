...
Starter files: search.py, test_search.py, analysis.ipynb Test command: pytest -v (from your Lab10 root)
...
Move search.py into src/
Move test_search.py into tests/
Move analysis.ipynb into notebooks/
Keep conftest.py in the Lab10 root
...
Track two boundaries: first (initially 0) and last (initially len(a_list) - 1)
While first <= last:
...
Paste your completed search functions (all four — original and counted versions) into the first code cell
...
Select your repo, set path to labs/Lab10/notebooks/analysis.ipynb
Click OK
git pull
