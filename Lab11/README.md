...
Starter files: sorting.py, test_sorting.py, analysis.ipynb Test command: pytest -v (from your Lab11 root)
...
Keep conftest.py in the Lab11 root
...
Make n - 1 passes through the list (where n is the length)
On each pass i (starting from 0), compare adjacent items from position 0 up to position n - 1 - i (because the last i items are already sorted)
If a_list[j] > a_list[j + 1], swap them
...
Start at position 1 (position 0 is already "sorted" by itself)
For each position i, save the current item in a variable (current_value)
Walk backward through the sorted region (positions i-1 down to 0):
...
Click File > Save a copy in Drive — this gives you an editable copy
Paste your completed sort functions (all five — three originals plus two counted versions) into the first code cell
Run each experiment cell, then answer the questions in the markdown cells below it
...
Select your repo, set path to labs/Lab11/notebooks/analysis.ipynb
Click OK
git pull
